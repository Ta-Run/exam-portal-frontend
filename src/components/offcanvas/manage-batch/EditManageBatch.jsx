...
import { reqToEditClientManageBatch } from "../../../reduxToolkit/services/assessmentServices";
...
const EditManageBatch = ({ show, handleClose, sectorDropDown, clientJobRoleDropDown, handleGetManageBatch, editData }) => {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      const response = await dispatch(reqToEditClientManageBatch(manageBatch));
  ...
  useEffect(() => {
    setManageBatch({
      ...
      TrainingPartnerName: editData?.TrainingPartnerName,
      TrainingCenterName: editData?.TrainingCenterName,
      TrainingPartnerEmail: editData?.TrainingPartnerEmail,
      TrainingCenterEmail: editData?.TrainingCenterEmail,
      StartDate: editData?.StartDate,
      StartTime: editData?.StartTime,
      EndDate: editData?.EndDate,
      EndTime: editData?.EndTime,
      BatchCode: editData?.BatchCode,
      TotalCandidate: editData?.TotalCandidate,
      photo: editData?.photo,
      video: editData?.video,
      PhotoCaptureMinute: editData?.PhotoCaptureMinute,
      videoCaptureMinute: editData?.videoCaptureMinute
    })
  }, [editData])
    ...
    <>
      ...
      >
        <div className="offcanvas-header que-header">
          <h5 className="offcanvas-title">Edit Batch</h5>
          <button
            type="button"
            onClick={handleClose}
            className="close-btn"
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                autoComplete="off"
                required
                value={formatDate(manageBatch?.StartDate) || ""}
                onChange={handleChange}
                min={minDate}
              />
            </div>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                autoComplete="off"
                required
                value={formatDate(manageBatch?.EndDate) || ""}
                onChange={handleChange}
                min={minDate}
              />
            </div>
...
export default EditManageBatch;